...
qbDir = '/home/charlesiglehart/Documents/Research/comparisonStudy/data/dtiSegmentation/segmentation'
...
#    st_rs_R_vsi  = np.column_stack((st_rs_R_vsi ,vsi ))
    ...
    qbFile = os.path.join(qbDir,'case'+caseStr+'_QB_L_DTI_ST_mask_reordered.nii.gz')
    rsFile = os.path.join(rsDir,'case'+caseStr+'_RS_L_DTI_reordered.nii.gz')
    dice,vsi = computeStats.computeDiceAndVSI(qbFile,rsFile,1)
    qb_rs_L_dice = np.row_stack((qb_rs_L_dice,dice))

    
    qbFile = os.path.join(qbDir,'case'+caseStr+'_QB_R_DTI_ST_mask_reordered.nii.gz')
    rsFile = os.path.join(rsDir,'case'+caseStr+'_RS_R_DTI_reordered.nii.gz')
    dice,vsi = computeStats.computeDiceAndVSI(qbFile,rsFile,1)
    qb_rs_R_dice = np.row_stack((qb_rs_R_dice,dice))
