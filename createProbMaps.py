...
dataPath = '/home/charlesiglehart/Documents/Research/comparisonStudy/data/oldSegmentation'
outputDir = '/home/charlesiglehart/Documents/Research/comparisonStudy/data/probMaps/'
outputName = 'rsProbMapRegroupedCombined.nii.gz'
...
for i in range(len(cases)):
    niiPath = os.path.join(dataPath,'case'+str(cases[i])+'_RG_L_TMP.nii.gz')
    nii = nib.load(niiPath)
    ims[:,:,:,i] = nii.dataobj.get_unscaled()
    niiPath = os.path.join(dataPath,'case'+str(cases[i])+'_RG_R_TMP.nii.gz')
    nii = nib.load(niiPath)
...
maxMap = np.zeros([dims[0],dims[1],dims[2]])
...
#                maxMap[i,j,k] = mode(counts[i,j,k,:]>0)
...
nib.save(niiOut,os.path.join(outputDir,outputName))